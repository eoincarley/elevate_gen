pro oplot_nrh_on_three_color, tstart, freq_tags=freq_tags, back_sub=back_sub
	...
	colors = reverse((indgen(9)+2.))	;reverse([6,8,4,10])	;[2, 3, 7, 10];(indgen(9)+2.)
					;colors = [6, 7, 10]
	inds = reverse(indgen(9))	;reverse([5,6,7,8])		;[0, 1, 5, 7] ;indgen(9)
	for j=0, n_elements(inds)-1 do begin
		...
		data_roi = nrh_data[64:127, 0:64] 	;nrh_data[0:32, 64:127] 	; For determinging source max for 2014-04-18 event
		max_val = max( (data_roi) ,/nan) 							   
		nlevels=5.0   

		top_percent = 0.85	; 0.3 on a linear scale for the 2014 Sep 01 event
