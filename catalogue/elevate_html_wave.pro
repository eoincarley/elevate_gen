pro elevate_html_wave, row_num, tstart, em_start, template, $
                        euv_wave, output_flare_data, assoc_wave_time

     restore, '~/ELEVATE/data/lmsal_euv_wave_times.sav'  ; Generated from aia_extract_wave_times.pro                    
	
	euv_wave = 'no' ;Default, prove otherwise.
    wave_check_t0 = anytim(em_start, /utim) - 60.0*60.0*2.0
    ...
    if result[0] ne -1 then begin
        FOR k =0, n_elements(result)-1 DO BEGIN
            ...
            ; This operates if flare data could not be found by other means e.g., Solar Monitor and other LMSAL lists.
